    ...
    $.get(url+"?page="+page, function(data, status){
        ...
        if (status == "success") {
            ...
            for(let i = 0; i<data.data.length;i++){
                ...
                dataPlanHTML += `<td>${formatValues('volume',data.data[i].volume)} for N${data.data[i].price}</td><td>${formatValues('validity',data.data[i].validity)}</td>`;
                dataPlanHTML += `<td><span class ="data-delete-link" onClick = "deleteDataPlan(${data.data[i].id},${data.current_page})">Delete</span> / <span class ="data-delete-link" onclick = "editDataPlan(${data.data[i].id})">Edit</span></td></tr>`;
