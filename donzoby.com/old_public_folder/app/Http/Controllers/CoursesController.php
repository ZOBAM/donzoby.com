    ...
    public function index($course=1,$subject=2,$id=0,$topic="2")//{course}/{subject?}/{id?}/{topic?}
    {
        ...
        if (in_array($course, $courses)) {//check if course in url is in array
            ...
            if (in_array($subject, $subjects)) {
				if ($id!=0 || $id == "data-plans") {//an id is provided
					if(is_numeric($id)){
					...
					//for data plans
						...
						$data_plans = Data_plan::get();
						$description = $courses_descriptions[$id];
						$title = $courses_titles[$id];
						$post_image = URL('public/images/donzoby-logo-wtbg.png');
						return view('courses.data-plans',compact('listed_subjects','posts','title','description','post_image','data_plans'));
					}
                    //$subject = 'HTML';
